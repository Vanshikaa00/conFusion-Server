...
.options(cors.corsWithOptions, (req, res) => { res.sendStatus(200) })
    ...
    .put(cors.corsWithOptions,authenticate.verifyUser, (req, res, next) => {
        Dishes.findById(req.params.dishId)
            .populate('comments.author')
            .then((dish) => {
                if (dish != null && dish.comments.id(req.params.commentId) != null && dish.comments.id(req.params.commentId).author == req.user.docs._id) {
                    if (req.body.rating) {
                    ...
                    err = new Error('Put operation not allowed');
                    err.status = 404;
                    return next(err);
                }
            }, (err) => next(err))
        ...
        Dishes.findById(req.params.dishId)
            .then((dish) => {
                if (dish != null && dish.comments.id(req.params.commentId) != null && dish.comments.id(req.params.commentId).author == req.user.id) {
                    ...
                    err = new Error('Delete operation not allowed');
                    err.status = 404;
                    return next(err);
                }
            }, (err) => next(err))
